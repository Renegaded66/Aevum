...
class InitialActivitySnapshotWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        // Bridge liegen, enqueuen wir den normalen Recognition-Worker, der
        // den Rest übernimmt (Confidence-Check, Auto-Start, SleepShield).
        ...
        if (vehicleCluster != null && vehicleCluster.durationMs >= MIN_PROBE_DURATION_MS) {
            Log.d(
                ...
                "Initial-Snapshot erkannte IN_VEHICLE-Cluster: " +
                    "${vehicleCluster.durationMs / 1000}s, confidence=${vehicleCluster.peakConfidence} → Auto-Start"
            )
            ...
            // Normalen Recognition-Worker enqueuen — der macht den Rest.
            androidx.work.OneTimeWorkRequestBuilder<ActivityRecognitionWorker>().build().also {
                androidx.work.WorkManager.getInstance(ctx).enqueue(it)
            }
        } else {
