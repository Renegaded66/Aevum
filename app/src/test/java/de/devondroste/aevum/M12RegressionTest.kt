...
class M12RegressionTest {
    ...
    fun autoSourcesContainsAllAutomaticTypes() {
        assertThat(de.devondroste.aevum.ui.screens.timeline.AUTO_SOURCES).containsExactly(
            "GEOFENCE_AUTO",
            "HEALTH_SLEEP_AUTO",
    ...
    fun timelineZoomIsWithinBounds() {
        val def = de.devondroste.aevum.ui.screens.timeline.TimelineUiState.DEFAULT_PIXELS_PER_HOUR
        val min = de.devondroste.aevum.ui.screens.timeline.TimelineUiState.MIN_PIXELS_PER_HOUR
        val max = de.devondroste.aevum.ui.screens.timeline.TimelineUiState.MAX_PIXELS_PER_HOUR
    ...
    fun manualAcceptStillUsesDefaultSourceType() = runBlocking {
        ...
        assertThat(result).isInstanceOf(de.devondroste.aevum.domain.automation.CandidateReviewResult.Accepted::class.java)
    ...
    private class FakeActivityRepository : ActivityRepository {
        ...
        override suspend fun insertWithTags(session: ActivitySession, tags: List<de.devondroste.aevum.data.model.Tag>) { inserted.add(session) }
    ...
    private class FakeChangeRepository : ActivitySessionChangeRepository {
        override fun getBySessionId(sessionId: String): Flow<List<de.devondroste.aevum.data.model.ActivitySessionChange>> = flowOf(emptyList())
        override fun getByTypeAndDateRange(type: String, start: Long, end: Long): Flow<List<de.devondroste.aevum.data.model.ActivitySessionChange>> = flowOf(emptyList())
        override fun getBySourceCandidateId(candidateId: String): Flow<List<de.devondroste.aevum.data.model.ActivitySessionChange>> = flowOf(emptyList())
        override suspend fun insert(change: de.devondroste.aevum.data.model.ActivitySessionChange) {}
        override suspend fun insertAll(changes: List<de.devondroste.aevum.data.model.ActivitySessionChange>) {}
    }
}
