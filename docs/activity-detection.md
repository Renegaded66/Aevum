...
| **ON_BICYCLE-EXIT-Artefakt während der Radfahrt** | **M18.135:** Der EXIT nimmt den ROHEN Kontext zurück (2× UNKNOWN, M18.117-Hysterese) — das Zweirad-Gate (`BikeContextGuard` → `currentMotionContext()`) hält die 12-m/s-Schwelle aber, solange ein BESTÄTIGTES Rad-Sample frisch ist (90 s = drei 30-s-AR-Takte). Gemessen vorher: 29 km/h → Auto-Session 15 s nach dem EXIT. Zusätzlich: kein Auto-Start über eine laufende `radfahren`-Session (DriveStartWorker-Guard), außer die Bewegung erreicht das 12-m/s-Gate (`isVehicleLevelMovement` — Motorrad ≥ 50 km/h, M18.130) |
| **Rad abgestellt, ins Auto gestiegen** | **M18.135:** Ein bestätigtes IN_VEHICLE-Sample (Confidence ≥ 60) verwirft die Rad-Frische sofort; mit dem nächsten Kontext-Sample gilt wieder die 8-m/s-Schwelle. Ist die Rad-Session bereits beendet, läuft der Auto-Start unverändert (M18.130-Bestandsschutz) |
| **Radfahrt endet (abgestellt + geht)** | **M18.135:** Die Walk-/Step-Stop-TRIGGER lesen den Live-Session-Zustand (`isLiveAutoTrackedSession`) statt `isDriveActive()` — bei einer Rad-Session ist dieses Flag false, der Trigger war deshalb geschlossen, obwohl beide Detektoren korrekt feuerten. Die Radfahrt endet jetzt über den echten Ausstiegs-Trigger (75-s-Gnadenfrist bzw. 10 Schritte), nicht erst über den 5-Minuten-Watchdog |
